		 ...
		 elseif(empty($text))
		...
		else
	          {
	             ...
	             if(mysqli_query($db, $sql)) {
	                 ...
	                 $user_query = "SELECT firstname, lastname FROM signup WHERE user_id = '$user_email'";
	                 $user_result = mysqli_query($db, $user_query);
	                 $user_data = mysqli_fetch_assoc($user_result);
	                 $user_name = $user_data['firstname'] . ' ' . $user_data['lastname'];
	                 
	                 // Create notification
	                 $notification_message = $user_name . " commented on recipe #" . $DISC . ": " . substr($text, 0, 50) . "...";
	                 $notification_sql = "INSERT INTO admin_notifications (type, message, reference_id) VALUES ('comment', ?, ?)";
	                 $stmt = mysqli_prepare($db, $notification_sql);
	                 mysqli_stmt_bind_param($stmt, "si", $notification_message, $DISC);
